...
class Subgroup extends CI_Controller {
	...
	function __construct()
	{
		...
		//global var
		$this->module_alias = "Subgroup";
		$this->module = 'accounting/subgroup';
	}
	...
	public function index()
	{
        ...
        $this->parser->parse("subgroup.tpl", $data);
	}
	
	public function table_list()
	{
		$this->datatables->select('b.nama,a.kode,a.nama as nama_subgroup,a.idsubgroup')
			->edit_column('a.idsubgroup', '$1', 'subgroup', 'callback_actions(a.idsubgroup)')
            ->from('ak_subgroup a')
			->join('ak_group b','a.idgroup=b.idgroup')
			;
        echo $this->datatables->generate();
	}
	
	static public function callback_actions($id){
		$module = 'accounting/subgroup';
	...
	public function add()
	{
		...
		if ($id != "") {
			$data['cond'] = "Edit";
			$where = array("idsubgroup"=>$id);
			$data['detail'] = $this->global_model->get_detail_data('ak_subgroup',$where);
		} else {
			$data['cond'] = "Add";
		}
		...
		$data['idgroup'] = form_dropdown('idgroup',$options,@$data['detail']['idgroup'],'class="select2"');
		
		$data['stat_cond'] = "insert_commit";
		$data['module_alias'] = $this->module_alias;
		$data['module'] = $this->module;
		// Load the template from the views directory
        $this->parser->parse("subgroup_add.tpl", $data);
	}
	...
	public function insert_commit()
	{
		...
		if ($this->input->post('idsubgroup') == "") { //insert cond
			//unset data
			unset($data['idsubgroup']);
			//set data
			$data['created_date'] = date("Y-m-d H:i:s");
			$data['created_by'] = $this->ion_auth->user()->row()->id;
			//call model
			$data = $this->global_model->insert('ak_subgroup',$data);
		} else { //edit cond
			//call model
			$where = array("idsubgroup"=>$data['idsubgroup']);
			$data = $this->global_model->update('ak_subgroup',$data,$where);
		}
	...
	{
		$data['icon'] = "fa-money";
		$data['bc'] = "Accounting <span>> Settings > Manage Subgroup > Add Subgroup</span>";
		$data['module_alias'] = "Subgroup";
		$data['module_name'] = 'accounting/subgroup';
		// Load the template from the views directory
	...
	public function delete()
	{
		...
		$id = $this->uri->segment(4);
		//call model
		$data = $this->global_model->delete('ak_subgroup','idsubgroup',$id);
		echo json_encode($data);
	}
}