...
class Center_support extends CI_Controller {
	...
	function __construct()
	{
		...
		//global var
		$this->module_alias = "Center Support";
		$this->module = 'reference/center_support';
	}
	...
	public function index()
	{
        ...
        $this->parser->parse("center_support.tpl", $data);
	}
	
	public function table_list()
	{
		$this->datatables->select('nilai,idref')
			->edit_column('idref', '$1', 'center_support', 'callback_actions(idref)')
            ->from('ref_center_support');
 
        echo $this->datatables->generate();
	}
	
	static public function callback_actions($id){
		$module = 'reference/center_support';
	...
	public function add()
	{
		...
		if ($id != "") {
			...
			$data['detail'] = $this->global_model->get_detail_data('ref_center_support',$where);
		} else {
			$data['cond'] = "Add";
		}
		...
		$data['module'] = $this->module;
		// Load the template from the views directory
        $this->parser->parse("center_support_add.tpl", $data);
	}
	...
	public function insert_commit()
	{
		...
		if ($this->input->post('idref') == "") { //insert cond
			...
			unset($data['idref']);
			//call model
			$data = $this->global_model->insert('ref_center_support',$data);
		} else { //edit cond
			//call model
			$where = array("idref"=>$data['idref']);
			$data = $this->global_model->update('ref_center_support',$data,$where);
		}
	...
	{
		$data['icon'] = "fa-wrench";
		$data['bc'] = "Reference <span>> Manage Center_support > Add Center_support</span>";
		$data['module_alias'] = "Center Support";
		$data['module_name'] = 'reference/center_support';
		// Load the template from the views directory
	...
	public function delete()
	{
		...
		$id = $this->uri->segment(4);
		//call model
		$data = $this->global_model->delete('ref_center_support','idref',$id);
		echo json_encode($data);
	}
}