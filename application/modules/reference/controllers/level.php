...
class Level extends CI_Controller {
	...
	function __construct()
	{
		...
		//global var
		$this->module_alias = "Level";
		$this->module = 'reference/level';
	}
	...
	public function index()
	{
        ...
        $this->parser->parse("level.tpl", $data);
	}
	
	public function table_list()
	{
	$this->datatables->select('nilai,idref')
			->edit_column('idref', '$1', 'level', 'callback_actions(idref)')
            ->from('ref_level');
	...
	public function add()
	{
		...
		if ($id != "") {
			...
			$data['detail'] = $this->global_model->get_detail_data('ref_level',$where);
		} else {
			$data['cond'] = "Add";
		}
		...
		$data['module'] = $this->module;
		// Load the template from the views directory
        $this->parser->parse("level_add.tpl", $data);
	}
	...
	public function insert_commit()
	{
		...
		if ($this->input->post('idref') == "") { //insert cond
			...
			unset($data['idref']);
			//call model
			$data = $this->global_model->insert('ref_level',$data);
		} else { //edit cond
			//call model
			$where = array("idref"=>$data['idref']);
			$data = $this->global_model->update('ref_level',$data,$where);
		}
	...
	{
		$data['icon'] = "fa-wrench";
		$data['bc'] = "Reference <span>> Manage Level > Add Level</span>";
		$data['module_alias'] = "Level";
		$data['module_name'] = 'reference/level';
		// Load the template from the views directory
	...
	public function delete()
	{
		...
		$id = $this->uri->segment(4);
		//call model
		$data = $this->global_model->delete('ref_level','idref',$id);
		echo json_encode($data);
	}
}