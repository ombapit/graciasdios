...
class Footplate extends CI_Controller {
	...
	function __construct()
	{
		...
		//global var
		$this->module_alias = "Footplate";
		$this->module = 'reference/footplate';
	}
	...
	public function index()
	{
        ...
        $this->parser->parse("footplate.tpl", $data);
	}
	
	public function table_list()
	{
		$this->datatables->select('nilai,idref')
			->edit_column('idref', '$1', 'footplate', 'callback_actions(idref)')
            ->from('ref_footplate');
	...
	public function add()
	{
		...
		if ($id != "") {
			...
			$data['detail'] = $this->global_model->get_detail_data('ref_footplate',$where);
		} else {
			$data['cond'] = "Add";
		}
		...
		$data['module'] = $this->module;
		// Load the template from the views directory
        $this->parser->parse("footplate_add.tpl", $data);
	}
	...
	public function insert_commit()
	{
		...
		if ($this->input->post('idref') == "") { //insert cond
			...
			unset($data['idref']);
			//call model
			$data = $this->global_model->insert('ref_footplate',$data);
		} else { //edit cond
			//call model
			$where = array("idref"=>$data['idref']);
			$data = $this->global_model->update('ref_footplate',$data,$where);
		}
	...
	{
		$data['icon'] = "fa-wrench";
		$data['bc'] = "Reference <span>> Manage Footplate > Add Footplate</span>";
		$data['module_alias'] = "Footplate";
		$data['module_name'] = 'reference/footplate';
		// Load the template from the views directory
	...
	public function delete()
	{
		...
		$id = $this->uri->segment(4);
		//call model
		$data = $this->global_model->delete('ref_footplate','idref',$id);
		echo json_encode($data);
	}
}