...
class Ambalan extends CI_Controller {
	...
	function __construct()
	{
		...
		//global var
		$this->module_alias = "Ambalan";
		$this->module = 'reference/ambalan';
	}
	...
	public function index()
	{
        ...
        $this->parser->parse("ambalan.tpl", $data);
	}
	
	public function table_list()
	{
	$this->datatables->select('nilai,idref')
			->edit_column('idref', '$1', 'ambalan', 'callback_actions(idref)')
            ->from('ref_ambalan');
	...
	public function add()
	{
		...
		if ($id != "") {
			...
			$data['detail'] = $this->global_model->get_detail_data('ref_ambalan',$where);
		} else {
			$data['cond'] = "Add";
		}
		...
		$data['module'] = $this->module;
		// Load the template from the views directory
        $this->parser->parse("ambalan_add.tpl", $data);
	}
	...
	public function insert_commit()
	{
		...
		if ($this->input->post('idref') == "") { //insert cond
			...
			unset($data['idref']);
			//call model
			$data = $this->global_model->insert('ref_ambalan',$data);
		} else { //edit cond
			//call model
			$where = array("idref"=>$data['idref']);
			$data = $this->global_model->update('ref_ambalan',$data,$where);
		}
	...
	{
		$data['icon'] = "fa-wrench";
		$data['bc'] = "Reference <span>> Manage Ambalan > Add Ambalan</span>";
		$data['module_alias'] = "Ambalan";
		$data['module_name'] = 'reference/ambalan';
		// Load the template from the views directory
	...
	public function delete()
	{
		...
		$id = $this->uri->segment(4);
		//call model
		$data = $this->global_model->delete('ref_ambalan','idref',$id);
		echo json_encode($data);
	}
}