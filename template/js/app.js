...
function confirm_delete(module,id) {
	// ask verification
	$.SmartMessageBox({
		title : "<i class='fa fa-trash-o' style='color:red'></i> Delete Confirmation",
		content : "<span id='block_msg'>Are you sure to delete this data?</span>",
		buttons : '[No][Yes]'
	}, function(ButtonPressed) {
		if (ButtonPressed == "Yes") {
...
function delete_data(module,id) {
	$.ajax({
		...
		success : function(data) {
			if (data.success) {
				window.location.hash = site_url + module;
