...
class Beam extends CI_Controller {
	...
	function __construct()
	{
		...
		//global var
		$this->module_alias = "Beam";
		$this->module = 'reference/beam';
	}
	...
	public function index()
	{
        ...
        $this->parser->parse("beam.tpl", $data);
	}
	
	public function table_list()
	{
		$this->datatables->select('nilai,satuan,idref')
			->edit_column('idref', '$1', 'beam', 'callback_actions(idref)')
            ->from('ref_beam');
	...
	public function add()
	{
		...
		if ($id != "") {
			...
			$data['detail'] = $this->global_model->get_detail_data('ref_beam',$where);
		} else {
			$data['cond'] = "Add";
		}
		...
		$data['module'] = $this->module;
		// Load the template from the views directory
        $this->parser->parse("beam_add.tpl", $data);
	}
	...
	public function insert_commit()
	{
		...
		if ($this->input->post('idref') == "") { //insert cond
			...
			unset($data['idref']);
			//call model
			$data = $this->global_model->insert('ref_beam',$data);
		} else { //edit cond
			//call model
			$where = array("idref"=>$data['idref']);
			$data = $this->global_model->update('ref_beam',$data,$where);
		}
	...
	{
		$data['icon'] = "fa-wrench";
		$data['bc'] = "Reference <span>> Manage Beam > Add Beam</span>";
		$data['module_alias'] = "Beam";
		$data['module_name'] = 'reference/beam';
		// Load the template from the views directory
	...
	public function delete()
	{
		...
		$id = $this->uri->segment(4);
		//call model
		$data = $this->global_model->delete('ref_beam','idref',$id);
		echo json_encode($data);
	}
}