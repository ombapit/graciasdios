...
class Accessories extends CI_Controller {
	...
	function __construct()
	{
		...
		//global var
		$this->module_alias = "Accessories";
		$this->module = 'reference/accessories';
	}
	...
	public function index()
	{
        ...
        $this->parser->parse("accessories.tpl", $data);
	}
	
	public function table_list()
	{
		$this->datatables->select('nilai,idref')
			->edit_column('idref', '$1', 'accessories', 'callback_actions(idref)')
            ->from('ref_accessories');
 
        echo $this->datatables->generate();
	}
	
	static public function callback_actions($id){
		$module = 'reference/accessories';
	...
	public function add()
	{
		...
		if ($id != "") {
			...
			$data['detail'] = $this->global_model->get_detail_data('ref_accessories',$where);
		} else {
			$data['cond'] = "Add";
		}
		...
		$data['module'] = $this->module;
		// Load the template from the views directory
        $this->parser->parse("accessories_add.tpl", $data);
	}
	...
	public function insert_commit()
	{
		...
		if ($this->input->post('idref') == "") { //insert cond
			...
			unset($data['idref']);
			//call model
			$data = $this->global_model->insert('ref_accessories',$data);
		} else { //edit cond
			//call model
			$where = array("idref"=>$data['idref']);
			$data = $this->global_model->update('ref_accessories',$data,$where);
		}
	...
	{
		$data['icon'] = "fa-wrench";
		$data['bc'] = "Reference <span>> Manage Accessories > Add Accessories</span>";
		$data['module_alias'] = "Accessories";
		$data['module_name'] = 'reference/accessories';
		// Load the template from the views directory
	...
	public function delete()
	{
		...
		$id = $this->uri->segment(4);
		//call model
		$data = $this->global_model->delete('ref_accessories','idref',$id);
		echo json_encode($data);
	}
}